...
import { Check, Clock, Coins, Eye, Loader2, X } from "lucide-react";
...
interface IncomingSessionRequestsPanelProps {
  token: string | null | undefined;
  enabled: boolean;
}

export default function IncomingSessionRequestsPanel({
  token,
  enabled,
  ...
  const handleReject = (r: IncomingSessionRequestItem) => {
    void (async () => {
      try {
        await rejectSessionRequest(token, r.request_id);
        toast({ title: "Request declined" });
        setDetail((d) => (d?.request_id === r.request_id ? null : d));
  ...
  const handleAccept = (r: IncomingSessionRequestItem) => {
    void (async () => {
      try {
        await acceptSessionRequest(token, r.request_id);
        toast({ title: "Session scheduled", description: "Credits charged to mentee wallet." });
        setDetail((d) => (d?.request_id === r.request_id ? null : d));
        invalidateAfterDecision();
          ...
          >
            <div className="min-w-0">
              ...
              <p className="text-xs text-muted-foreground">
                {format(parseISO(r.start_time), "EEE MMM d · h:mm a")}
                <span className="mx-1">·</span>
                {r.agreed_cost} credits if accepted
              </p>
            </div>
                ...
                <dl className="space-y-3.5 text-sm">
                  ...
                  <div>
                    ...
                    <dd className="mt-0.5 text-foreground">
                      {format(parseISO(detail.start_time), "EEEE, MMMM d, yyyy")}
                    </dd>
                    <dd className="text-muted-foreground">
                      {format(parseISO(detail.start_time), "h:mm a")}
                      {" · "}
                      {format(parseISO(detail.end_time), "h:mm a")}
                    </dd>
                  </div>
                  <div>
                    <dt className="flex items-center gap-1.5 text-xs font-medium uppercase tracking-wide text-muted-foreground">
                      <Coins className="h-3.5 w-3.5" /> Credits if you accept
                    </dt>
                    <dd className="mt-0.5 font-medium tabular-nums text-foreground">
                      {detail.agreed_cost} credits
                    </dd>
                    <dd className="mt-1 text-xs text-muted-foreground">
                      Charged to the mentee when you accept (BOOK_MENTOR_SESSION rule).
                    </dd>
                  </div>
                  <div>
                    <dt className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
